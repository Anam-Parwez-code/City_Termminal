...
const TrackingScreen = ({ navigation, route }) => {
  ...
  const markerCoordinate = useRef(
    new AnimatedRegion({
      latitude: DEFAULT_DRIVER_LOCATION.latitude,
      longitude: DEFAULT_DRIVER_LOCATION.longitude,
      latitudeDelta: 0,
      longitudeDelta: 0,
    })
  ).current;
  ...
  const routeLine = useMemo(
    () => [driverLocation, destination].filter(isValidCoordinate),
  ...
  useEffect(() => {
    ...
    socket.on('location_update', (payload) => {
      ...
      const nextCoordinate = toCoordinate(payload);
      moveDriver(nextCoordinate);
    });
  ...
  useEffect(() => {
    if (mapRef.current && routeLine.length > 1) {
      mapRef.current.fitToCoordinates(routeLine, {
        edgePadding: { top: 110, right: 48, bottom: 280, left: 48 },
        animated: true,
      });
    }
  }, [routeLine]);
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Marker.Animated coordinate={markerCoordinate} title="Luggage van">
      ...
      <Animated.View style={[styles.sheet, { transform: [{ translateY: sheetTranslateY }] }]}>
        ...
        >
          <Text style={styles.primaryButtonText}>Re-center route</Text>
        </TouchableOpacity>
      </Animated.View>
    </View>
  );
